...
export class AppMenuitemComponent implements OnInit, OnDestroy {
  ...
  itemClick(event: Event) {
    ...
    if ((this.root && this.isSlim) || this.isHorizontal) {
      this.apolloLayoutService.state.menuHoverActive =
        !this.apolloLayoutService.state.menuHoverActive;
    }
    ...
    if (this.item.items) {
      ...
      if (this.root && this.active && (this.isSlim || this.isHorizontal)) {
        this.apolloLayoutService.onOverlaySubmenuOpen();
      }
    } else {
      if (this.apolloLayoutService.isMobile()) {
  ...
  onMouseEnter() {
    ...
    if (
      ...
      this.apolloLayoutService.isDesktop()
    ) {
      if (this.apolloLayoutService.state.menuHoverActive) {
        this.active = true;
        this.menuService.onMenuStateChange({ key: this.key });
      }
    }
  ...
  get isHorizontal() {
    return this.apolloLayoutService.isHorizontal();
  }

  get isSlim() {
    return this.apolloLayoutService.isSlim();
  }
